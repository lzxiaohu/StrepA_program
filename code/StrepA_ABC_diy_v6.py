...
# synthetic data
_Tdry = simulate_prevalence_v5_numba(np.array([2.0, 1.5], float), fixed_params, seed=int(123))
T = _Tdry.size
print("T's size", T)
_Tdry1 = simulate_prevalence_v5_numba(np.array([2.0, 1.5], float), fixed_params, seed=int(123))
...
def summary_stats(series_2d):
    ...
    # avg_npmi_obs = avg_npmi_numpy(series_2d)
    # div_all_isolates_obs = ss.div_all_isolates_numpy(series_2d)

    return np.array(
        [avg_time_obs, max_time_obs, num_strains_obs, avg_time_repeat_obs, var_time_repeat_obs, avg_prev_obs], float)
...
def prior_value(R0_range, sigma_range, rng):
...
def select_epsilon(R0_range, sigma_range, s_obs, scale, n_pilot=5000, quantile=0.02, seed=123):
    ...
    for ii in range(n_pilot):
        R0_sel, sigma_sel = prior_value(R0_range, sigma_range, rng)
        y_sim = simulate_prevalence_v5_numba([R0_sel, sigma_sel], fixed_params, seed)
...
R0_range= [0., 5.0]
sigma_range = [0.1, 2.0]
eps, pilots = select_epsilon(R0_range, sigma_range, s_obs_v5_numba, scale, n_pilot=500, quantile=0.2, seed=123)
# eps = 1.513
print("eps: ", eps)
...
# function: abc_rejct
def abc_reject(R0_range, sigma_range, s_obs, scale, eps, n_accept=2000, max_trials=2_000_000, seed=123):
    rng = np.random.default_rng(123)
    accepted = []
    dists_acc = []
...
post, dists_acc, trials = abc_reject(R0_range, sigma_range, s_obs_v5_numba, scale, eps, n_accept=400, max_trials=2_000_000, seed=123)
...
print("Posterior mean b: ", sigma_samps.mean())
end = time.perf_counter()
print(f"Elapsed: {end - start:.4f} s")
# print("R0: ",  R0_samps)
# print("sigma: ", sigma_samps)
np.savetxt("./experimental_data/R0_samps_v06.csv", R0_samps, delimiter=",")
np.savetxt("./experimental_data/sigma_samps_v06.csv", sigma_samps, delimiter=",")
np.savetxt("./experimental_data/dists_acc_v06.csv", dists_acc, delimiter=",")
