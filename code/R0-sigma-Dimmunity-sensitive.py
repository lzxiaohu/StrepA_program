# filename: R0-sigma-Dimmunity-sensitive.py
...
while ii <= R0_range[1]:
    ...
    while jj <= sigma_range[1]:
        ...
        while kk <= Dimmunity_range[1]:
            y_sim = simulate_prevalence_v5_numba(np.array([ii, jj, kk*52.14], float), fixed_params, seed=int(123))
...
np.savetxt("./experimental_data/R0_sigma_Dimmunity_results_v2.csv", results, delimiter=",", header=header, comments="")
