...
Nstrains = 20       # number of strains: 42
...
def summary_stats(series_2d):
    ...
    return np.array(
        [avg_prev_obs, var_prev_obs, avg_div_obs, max_abundance_obs,
         avg_npmi_obs], float)


# synthetic data
if core_params_num == 2:
    _Tdry = simulate_prevalence_v5_numba(np.array([4.0, 0.5], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([4.0, 0.5], float), fixed_params, core_params_num, seed=int(123))
    print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
elif core_params_num == 3:
    _Tdry = simulate_prevalence_v5_numba(np.array([4.0, 0.5, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([4.0, 0.5, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
...
scale = np.array([220.0, 12000, 18, 60, 0.8], dtype=float)
print("scale", scale)
...
if core_params_num == 2:
    ...
    np.savetxt("../../experimental_data/from_260305/R0_samps_2params_R04p0.csv", R0_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/sigma_samps_2params_R04p0.csv", sigma_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/dists_acc_2params_R04p0.csv", dists_acc, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/ss_2params_R04p0.csv", ss, delimiter=",")
    ...
    np.savetxt("../../experimental_data/from_260305/R0_samps_3params_R04p0.csv", R0_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/sigma_samps_3params_R04p0.csv", sigma_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/Dimmunity_samps_3params_R04p0.csv", Dimmunity_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/dists_acc_3params_R04p0.csv", dists_acc, delimiter=",")
    np.savetxt("../../experimental_data/from_260305/ss_3params_R04p0.csv", ss, delimiter=",")
else:
    raise ValueError('Invalid core params num')
