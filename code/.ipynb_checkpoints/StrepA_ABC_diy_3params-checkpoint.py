...
# synthetic data
_Tdry = simulate_prevalence_v5_numba(np.array([2.0, 0.8, 2.0*52.14], float), fixed_params, seed=int(123))
T = _Tdry.size
print("T's size", T)
_Tdry1 = simulate_prevalence_v5_numba(np.array([2.0, 0.8, 2.0*52.14], float), fixed_params, seed=int(123))
...
scale = np.array([23.0, 350.0, 30000.0, 20.0, 1.0], dtype=float)
print("scale", scale)
...
post, dists_acc, trials = abc_reject(R0_range, sigma_range, Dimmunity_range, s_obs_v5_numba, scale, eps, n_accept=400, max_trials=2_000_000, seed=123)
...
np.savetxt("./experimental_data/Dimmunity_samps_3stars_v1.csv", sigma_samps, delimiter=",")
