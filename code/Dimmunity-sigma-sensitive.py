# filename: theta-sigma_sensitive.py
...
# test functions:
# synthetic data
_Tdry = simulate_prevalence_v5_numba(np.array([0.4 * 52.14, 0.8], float), fixed_params, seed=int(123))
T = _Tdry.size
print("T's size", T)
_Tdry1 = simulate_prevalence_v5_numba(np.array([0.4 * 52.14, 0.8], float), fixed_params, seed=int(123))
...
ii = Dimmunity_range[0]
ii_Dimmunity = 0.05
jj_sigma = 0.10
rows = []
while ii <= Dimmunity_range[1]:

    jj = sigma_range[0]
    while jj <= sigma_range[1]:
        y_sim = simulate_prevalence_v5_numba(np.array([ii * 52.14, jj], float), fixed_params, seed=int(123))
...
header = ",".join(["Dimmunity","sigma"] + sum_names)
np.savetxt("./experimental_data/Dimmunity_sigma_results_v2.csv", results, delimiter=",", header=header, comments="")
