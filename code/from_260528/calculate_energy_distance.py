...
def energy_distance(X, Y):
    ...
    # 2 * E[|X - Y|]: mean distance between X and Y
    # For each x in X, compute distance to each y in Y
    cross_dist = np.mean(
        np.sqrt(np.sum((X[:, None, :] - Y[None, :, :]) ** 2, axis=-1))
    )
    
    # E[|X - X'|]: mean pairwise distance within X
    xx_dist = np.mean(
        np.sqrt(np.sum((X[:, None, :] - X[None, :, :]) ** 2, axis=-1))
    )
    
    # E[|Y - Y'|]: mean pairwise distance within Y
    yy_dist = np.mean(
...
def calculate_energy_distance_abc(
    ...
    percentile=1.0,
    n_subsample=1000,    # Subsample for efficiency (pairwise is O(n^2))
    random_seed=42
):
    """
    Calculate energy distance between posterior (accepted samples) and prior (all samples).
    
    Large energy distance = ABC learned a lot from the data (posterior very different from prior)
    Small energy distance = ABC learned little (posterior similar to prior)
    ...
    n_subsample : int
        Number of samples from each distribution (O(n^2) computation)
    random_seed : int
        Random seed for reproducibility
    
    Returns:
    --------
    float : Energy distance between posterior and prior
    """
    ...
    # -------------------------------------------------------------------------
    # Subsample both for efficiency
    # -------------------------------------------------------------------------
    print(f"\nSubsampling to {n_subsample} each for efficiency...")
    ...
    Y = np.column_stack([prior_R0, prior_sigma])  # prior
...
if __name__ == "__main__":
    ...
    # Calculate energy distance: posterior vs prior
    ...
    print(f"\n✅ Energy distance (posterior vs prior): {ed:.6f}")
    ...
    peak_R0, peak_sigma = plot_energy_distance(
        ...
        title="ABC Posterior",
        save_path='../../figures/from_260528/ppc/observations/',
        save_filename='posterior_scatter_contour_1percentile.png'
    )
    
    print(f"\n✅ Peak density: R0={peak_R0:.4f}, sigma={peak_sigma:.4f}")
    ...
    peak_R0, peak_sigma = plot_energy_distance(
        ...
        title="ABC Posterior",
        save_path='../../figures/from_260528/ppc/observations/',
        save_filename='posterior_scatter_contour_0p1percentile.png'
    )
    ...
    peak_R0, peak_sigma = plot_energy_distance(
        ...
        title="ABC Posterior",
        save_path='../../figures/from_260528/ppc/observations/',
        save_filename='posterior_scatter_contour_0p01percentile.png'
    )
