...
# ============================================================================
if core_params_num == 2:
    _Tdry = simulate_prevalence_v5_numba(np.array([3.5, 0.3], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([3.5, 0.3], float), fixed_params, core_params_num, seed=int(123))
    print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
elif core_params_num == 3:
    _Tdry = simulate_prevalence_v5_numba(np.array([3.5, 0.3, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([3.5, 0.3, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
else:
    raise ValueError('Invalid core params num')
...
# VISUALIZE RESULTS
# ============================================================================
heatmap(y_obs_array, "Observed", save_path="../../figures/from_260312/ppc/sigma0p3/R03p5/")
heatmap(pred_mean, "PPC mean", save_path="../../figures/from_260312/ppc/sigma0p3/R03p5/")
heatmap(pred_mean - y_obs_array, "Error (mean - obs)", save_path="../../figures/from_260312/ppc/sigma0p3/R03p5/")

# ============================================================================
# ADDITIONAL DIAGNOSTICS (OPTIONAL)
# ============================================================================
# Uncertainty heatmap
heatmap(pred_hi - pred_lo, "90% CI Width (Uncertainty)", save_path="../../figures/from_260312/ppc/sigma0p3/R03p5/")

# Coverage map
heatmap(inside.astype(float), "Coverage Map (1=inside, 0=outside)", save_path="../../figures/from_260312/ppc/sigma0p3/R03p5/")
...
print("=" * 60)
print(f"Ground truth:      R0 = 3.5, sigma = 0.3")
