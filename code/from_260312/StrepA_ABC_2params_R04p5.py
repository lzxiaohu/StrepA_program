# file name: StrepA_ABC_2params_R04p5.py
...
# synthetic data
if core_params_num == 2:
    _Tdry = simulate_prevalence_v5_numba(np.array([4.5, 0.8], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    logging.info(f"T's size: {T}")
    # print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([4.5, 0.8], float), fixed_params, core_params_num, seed=int(123))
    ...
    _Tdry = simulate_prevalence_v5_numba(np.array([4.5, 0.8, 0.25 * 52.14], float), fixed_params, core_params_num,
                                         seed=int(123))
    T = _Tdry.size
    logging.info(f"T's size: {T}")
    # print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([4.5, 0.8, 0.25 * 52.14], float), fixed_params, core_params_num,
                                          seed=int(123))
    logging.info(f"allclose={np.allclose(_Tdry, _Tdry1)}, same_shape={_Tdry.shape == _Tdry1.shape}")
    # print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
...
def select_epsilon_3params(R0_range, sigma_range, Dimmunity_range, s_obs, scale, n_pilot=5000, quantile=0.02, seed=123):
    ...
    for ii in range(n_pilot):
        ...
        y_sim = simulate_prevalence_v5_numba([R0_sel, sigma_sel, Dimmunity_sel * 52.14], fixed_params, core_params_num,
                                             seed)
...
def abc_reject_3params(R0_range, sigma_range, Dimmunity_range, core_params_num, s_obs, scale, eps, n_accept=2000,
...
def abc_reject_2params(R0_range, sigma_range, core_params_num, s_obs, scale, eps, n_accept=2000, max_trials=2_000_000,
                       seed=123):
    rng = np.random.default_rng(123)
    accepted = []
    ss = []
...
if core_params_num == 2:
    ...
    np.savetxt("../../experimental_data/from_260312/R0_samps_2params_R04p5.csv", R0_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/sigma_samps_2params_R04p5.csv", sigma_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/dists_acc_2params_R04p5.csv", dists_acc, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/ss_2params_R04p5.csv", ss, delimiter=",")
    ...
    post, dists_acc, trials, ss = abc_reject_3params(R0_range, sigma_range, Dimmunity_range, core_params_num,
                                                     s_obs_v5_numba, scale, eps, n_accept=4000,
                                                     max_trials=2_000_000, seed=123)
    logging.info(f"Accepted: {len(post)}; Trials: {trials}; Acceptance rate: {len(post) / trials}")
    # print("Accepted: ", len(post), "Trials: ", trials, "Acceptance rate: ", len(post) / trials)
    ...
    np.savetxt("../../experimental_data/from_260312/R0_samps_3params_R04p5.csv", R0_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/sigma_samps_3params_R04p5.csv", sigma_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/Dimmunity_samps_3params_R04p5.csv", Dimmunity_samps, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/dists_acc_3params_R04p5.csv", dists_acc, delimiter=",")
    np.savetxt("../../experimental_data/from_260312/ss_3params_R04p5.csv", ss, delimiter=",")
else:
    raise ValueError('Invalid core params num')
