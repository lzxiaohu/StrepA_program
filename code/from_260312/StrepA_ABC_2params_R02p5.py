...
# synthetic data
if core_params_num == 2:
    _Tdry = simulate_prevalence_v5_numba(np.array([2.5, 0.5], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    logging.info(f"T's size: {T}")
    # print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([2.5, 0.5], float), fixed_params, core_params_num, seed=int(123))
    ...
    _Tdry = simulate_prevalence_v5_numba(np.array([2.5, 0.5, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    T = _Tdry.size
    logging.info(f"T's size: {T}")
    # print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([2.5, 0.5, 0.25 * 52.14], float), fixed_params, core_params_num, seed=int(123))
    logging.info(f"allclose={np.allclose(_Tdry, _Tdry1)}, same_shape={_Tdry.shape == _Tdry1.shape}")
    # print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
...
if core_params_num == 2:
    ...
    post, dists_acc, trials, ss = abc_reject_2params(R0_range, sigma_range, core_params_num, s_obs_v5_numba,
                                                     scale, eps, n_accept=5000, max_trials=2_000_000,
                                                     seed=123)
    ...
    post, dists_acc, trials, ss = abc_reject_3params(R0_range, sigma_range, Dimmunity_range, core_params_num,
                                                     s_obs_v5_numba, scale, eps, n_accept=5000,
