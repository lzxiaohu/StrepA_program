...
if core_params_num == 2:
    ...
    _Tdry = simulate_prevalence_v5_numba(np.array([1.5, 0.8, 0.25 * 52.14], float), fixed_params, core_params_num,
                                         seed=int(123))
    T = _Tdry.size
    logging.info(f"T's size: {T}")
    # print("T's size", T)
    _Tdry1 = simulate_prevalence_v5_numba(np.array([1.5, 0.8, 0.25 * 52.14], float), fixed_params, core_params_num,
                                          seed=int(123))
    logging.info(f"allclose={np.allclose(_Tdry, _Tdry1)}, same_shape={_Tdry.shape == _Tdry1.shape}")
    # print(np.allclose(_Tdry, _Tdry1), _Tdry.shape == _Tdry1.shape)
