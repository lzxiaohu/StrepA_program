# filename: R0-Dimmunity-sensitive.py
...
# sigma_range = [0.1, 1.0]
sigma = 0.8
fixed_params = np.array([DurationSimulation, Nstrains, sigma, omega,
...
def build_params(theta, fixed_params):
    ...
    R0, Dimmunity = float(theta[0]), float(theta[1])
    return np.array([
        fixed_params[0], fixed_params[1], Dimmunity, fixed_params[2],
...
# test functions:
# synthetic data
_Tdry = simulate_prevalence_v5_numba(np.array([2.0, 0.4*52.14], float), fixed_params, seed=int(123))
T = _Tdry.size
print("T's size", T)
_Tdry1 = simulate_prevalence_v5_numba(np.array([2.0, 0.4*52.14], float), fixed_params, seed=int(123))
...
while ii <= R0_range[1]:
    ...
    jj = Dimmunity_range[0]
    while jj <= Dimmunity_range[1]:
        y_sim = simulate_prevalence_v5_numba(np.array([ii, jj*52.14], float), fixed_params, seed=int(123))
...
header = ",".join(["R0","Dimmunity"] + sum_names)
np.savetxt("./experimental_data/R0_Dimmunity_results_v2.csv", results, delimiter=",", header=header, comments="")
